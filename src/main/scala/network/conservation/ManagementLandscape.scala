...
case class ManagementLandscape(
  ...
  grid: SquareGrid  // Keep reference to grid for neighbor lookups
):
...
object ManagementLandscape:
  ...
  def apply(landscapeGrid: SquareGrid, populations: Seq[Population], rnd: Random): ManagementLandscape =
    ...
    ManagementLandscape(areas, landscapeGrid)
